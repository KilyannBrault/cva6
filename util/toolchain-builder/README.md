...
> Please note that additionnal libraries that are not listed **may be** necessary to install.
...
Additionally, after getting the GCC toolchain, you can apply [`gcc-cva6-tune.patch`](./gcc-cva6-tune.patch) to add support for the `-mtune=cva6` option in GCC.
 ...
 * [`get-toolchain.sh`](./get-toolchain.sh): script in charge of obtaining the source code and extracting the correct code baselines.
 * [`build-toolchain.sh`](./build-toolchain.sh): script in charge of building and installing the different toolchain components in suitable order.
 * [`config/`](./config/): directory containing the configuration files for the various configurations.
 ...
 * the toolchain configuration name must match one of the predefined `config/CONFIG_NAME.sh` files under [`config`](./config) directory.

 * the installation location can be an arbitrary path.  It needs not to exist yet: any missing directories will be created during the building process. 
    > Note: The user running the [`build-toolchain.sh`](./build-toolchain.sh) script **must have** sufficient permissions to create the missing directories of the installation location.
...
If the name of the toolchain configuration is omitted, a default configuration will be selected implicitly.  _The default configuration is currently named [`gcc-13.1.0-baremetal`](./config/gcc-13.1.0-baremetal.sh)_ and builds a toolchain containing
...
The [`build-toolchain.sh`](./build-toolchain.sh) script incorporates fallbacks for several commonly encountered configuration and build issues. However, it is not meant to auto-detect major reconfigurations of source code such as a change of baseline configuration. _Whenever the source configuration is changed, please use the `-f` (or `--force`)
...
New configurations can be easily introduced by copying existing configuration files in subdirectory [`config/`](./config/) under a different name and adjusting the values of per-component variables. Taking `GCC` as an example:
 ...
 * `GCC_COMMIT` identifies the revision of source code to use: a specific commit, tag, or branch.
    > Note: If you set `GCC_COMMIT` to the name of a branch, the [`get-toolchain.sh`](./get-toolchain.sh) will update the local repository to the tip of the remote branch at every invocation.

 * `GCC_CONFIGURE_OPTS` is the list of options to pass to the configure script. It is located in [`config/global.sh`](./config/global.sh).
